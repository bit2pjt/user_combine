...
public class MmlController {
	...
	@Inject
	public MmlController(MmlService mmlService) {
...
////////////////
	...
	public String mmlWriteAction(Mml_ContentVO mmlContentVO) throws Exception {
		...
		mmlContentVO.setMml_title(mmlContentVO.getMml_title().trim());
			...
			System.out.println("===== InsertmmlContentVOResult ===== " + result);
			
			if (result==1) {
				//작성자의 개인 mmlList로 이동하게 추후 링크조정
				int mml_num = mmlService.getMmlContentNum(mmlContentVO.getId());
